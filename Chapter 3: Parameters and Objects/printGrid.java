...
The numbers count up from 1 to (rows x cols). The output are displayed in column-major order, meaning that the numbers shown increase sequentially down each column and wrap to the top of the next column to the right once the bottom of the current column is reached. 
...
public static void printGrid (int rows, int cols) {
    ...
    for (int i = 1; i <= rows; i++) {
        ...
        for (int j = 1; j < cols; j++) {
            System.out.print(", " + (currentVal + rows));
            currentVal += rows;
        }
        System.out.println();
    }    
